...
COPY node0 /mydata
RUN ./install_node.sh
RUN echo "SystemProxy address:" $(cat /mydata/build/systemcontract/output/SystemProxy.address)
run echo "God" $(cat /mydata/dependencies/cert/godInfo.txt | grep address)
run echo "God" $(cat /mydata/dependencies/cert/godInfo.txt | grep publicKey)
run echo "God" $(cat /mydata/dependencies/cert/godInfo.txt | grep privateKey)
EXPOSE 8545 30303 8821
CMD ["tail", "-f", "/dev/null"]